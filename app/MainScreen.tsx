...
import { CombatProvider, CombatHUD, Hitbox } from './combat/CombatContext';
import { 
  EffectComposer, 
  Bloom, 
  Vignette, 
...
// Arena Model component to load and display the arena
function ArenaModel() {
  const { scene } = useGLTF('/models/env/arena.glb');
  
  useEffect(() => {
    if (scene) {
      // Set up shadows for all meshes in the arena
      scene.traverse((node) => {
        if (node instanceof THREE.Mesh) {
          node.castShadow = true;
          node.receiveShadow = true;
  ...
  return (
    <primitive 
      ...
      position={[0, 0, 0]} 
      rotation={[0, 0, 0]} 
      scale={20} // Scaled up 20x as requested for a truly massive arena
    />
  );
}

// Preload the arena model
...
export function EldenRingScene() {
  return (
    <>
      ...
      {/* Load the boss character */}
      <Boss />
      
      {/* Dimmer ambient lighting for a more dramatic, imposing atmosphere */}
      <ambientLight intensity={0.8} color="#d9c7a5" />
      
      {/* Dramatic directional lighting to cast long shadows across the arena */}
      <directionalLight 
        position={[80, 120, -60]} 
        intensity={1.4} 
        color="#ff9d4d" 
      ...
      {/* Enhanced hemisphere light for ominous arena illumination */}
      <hemisphereLight intensity={0.5} color="#e8c481" groundColor="#252117" />
...
export function MainScreen() {
  return (
    <CombatProvider>
      ...
      {/* Health UI displays with bars for character and boss */}
      <CombatHUD />
      </div>
    </CombatProvider>
  );
